...
def ParseList():
    newList = []
    for i in range(0, len(allBlocks), 1):
        bX = allBlocks[i][0]
        bY = allBlocks[i][1]
        if f"{bX},{bY}:{bX + 1},{bY}" not in newList:
            newList.append(f"{bX},{bY}:{bX + 1},{bY}")
        if f"{bX + 1},{bY}:{bX + 1},{bY + 1}" not in newList:
            newList.append(f"{bX + 1},{bY}:{bX + 1},{bY + 1}")
        if f"{bX},{bY}:{bX},{bY + 1}" not in newList:
            newList.append(f"{bX},{bY}:{bX},{bY + 1}")
        if f"{bX},{bY + 1}:{bX + 1},{bY + 1}" not in newList:
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONUP:
            ...
            allGUIBlocks.append([round_to_multiple(x-15, 30), round_to_multiple(y-15, 30)])
            blockX = int(round((x-15)/30) - 8 + 100)
            blockY = int((round((y-15)/30) - 10) * -1 + 100)
            if [blockX, blockY] not in allBlocks:
